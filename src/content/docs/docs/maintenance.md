...
Accessibility is the easiest pillar to regress on without detection, because the changes that break it tend to be visual ones that pass a sighted-user review. When a screen is redesigned, a new control is added, or a view hierarchy is refactored, the following checks should be performed:
